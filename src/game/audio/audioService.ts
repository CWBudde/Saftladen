...
export type AudioSfxName = 'slice' | 'miss' | 'bomb' | 'game-over' | 'power-up'
...
function createSfxPack(sfxVolume: number): { pack: SfxPack; urls: string[] } {
  ...
  return {
    urls,
    pack: {
      slice: new Howl({ src: [urls[0]], volume: sfxVolume }),
      miss: new Howl({ src: [urls[1]], volume: sfxVolume }),
      bomb: new Howl({ src: [urls[2]], volume: Math.min(1, sfxVolume + 0.05) }),
      'game-over': new Howl({ src: [urls[3]], volume: sfxVolume }),
      'power-up': new Howl({ src: [urls[4]], volume: sfxVolume }),
    },
  }
}
